...
sinigang = StringVar()
entry2 = Entry(exam,bd=3,textvariable=sinigang)
entry2.grid(row=1, column = 1)

label3 = Label(exam,text = "Enter the second number:")
label3.grid(row=2, column=0)
adobo = StringVar()
entry3 = Entry(exam,bd=3,textvariable=adobo)
entry3.grid(row=2,column=1)

label4 = Label(exam,text="Enter the third number:")
label4.grid(row=3,column =0, sticky=W)
paksiw = StringVar()
entry4 = Entry(exam,bd=3,textvariable=paksiw)
...
tinola = StringVar()
entry5 = Entry(exam,bd=3,state="readonly",textvariable=tinola)
